...
#define STATUS_ERROR 13
#define STATUS_PLAYING 12
...
#define EEPROM_TRACK_0 0
#define EEPROM_TRACK_1 1
...
void setup() {
	...
	if (!volume.init(&card)) {
		Serial.println("Error: Volume ini");
		isError = true;
	}
	...
	if (isError) {
		digitalWrite(STATUS_ERROR, HIGH);
		// for debug window
		delay(500);
	}
...
void loop(){
	int number = 0;
	number += EEPROM.read(EEPROM_TRACK_0) * 10;
	number += EEPROM.read(EEPROM_TRACK_1);
	...
	sprintf(trackName, "track%03d.mp3", number);
	...
	Serial.println(number);
	number++;
	EEPROM.write(EEPROM_TRACK_0, (byte)(number / 10 % 10));
	EEPROM.write(EEPROM_TRACK_1, (byte)(number % 10));

	if (number > TRACK_COUNT) {
		Serial.println("Whoa there cowboy!"); 	
		delay(200);
		EEPROM.write(EEPROM_TRACK_0, 0);
		EEPROM.write(EEPROM_TRACK_1, 1);
		// initialize
		setup();
	}
}	
...
void playMP3(char* fileName) {
	...
	while(1) {
		while(!digitalRead(MP3_DREQ)) { 
			...
			if (count < 4) {
				digitalWrite(STATUS_PLAYING, HIGH);
				count++;
			}
			else if (count < 8) {
				digitalWrite(STATUS_PLAYING, LOW);
				count++;
			}
			else count = 0;
		}
